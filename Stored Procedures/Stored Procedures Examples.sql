--1) Stored Procedure to Update Staff Email:
		CREATE PROCEDURE SP_UpdateStaffEmail
						 @staff_id INT,
...
-- Usage
		...
		EXEC SP_UpdateStaffEmail 101, 'newemail@example.com';

		EXEC SP_UpdateStaffEmail 
				@staff_id = 101, 
				@new_email = 'newemail@example.com';

/* 
--This stored procedure updates a staff member's email by their ID  
--and prints a success message if the update is successful or an error 
--message if the staff member is not found.
*/

		
--#####################################################################
--2) Stored Procedure to Retrieve product stock information for a specific store
		CREATE PROCEDURE SP_GetProductStockByStore
						 @store_id INT
		AS
		BEGIN
...
-- Usage
		EXEC SP_GetProductStockByStore 2;

		EXEC SP_GetProductStockByStore 
			 @store_id =2;
...
--#####################################################################
----6) Stored Procedure to Get Stock Quantity
		CREATE PROCEDURE SP_GetStockQuantity 
						 @store_id INT,
						 @product_id INT
		AS
		BEGIN
...
-- Usage
		...
		EXEC SP_GetStockQuantityByStoreIdANDproductId
			 @store_id = 1,
			 @product_id = 10;
...
--8) Stored Procedure To Retrieve Staff Information by Store and Active Status
		CREATE PROCEDURE SP_GetActiveStaffByStore
						 @store_id INT,
						 @active TINYINT
		AS
		BEGIN
...
--9)tored Procedure To Retrieve Products by Brand Name and Model Year Range
		CREATE PROCEDURE SP_GetProductsByBrandAndYear
		...
		BEGIN
			...
			WHERE b.brand_name = @brand_name
			  AND p.model_year BETWEEN @start_year AND @end_year
...
--10)Stored Procedure To Retrieve Customers by City and State
		CREATE PROCEDURE SP_GetCustomersByLocation
						 @city VARCHAR(255),
						 @state VARCHAR(25)
		AS
		BEGIN
...
--11)Stored Procedure To Retrieve Product Information and Count
		CREATE PROCEDURE SP_FindProductByName
...
--12)Stored Procedure To Retrieve Product Information and Count
		CREATE PROCEDURE SP_GetOrdersByStatusAndDate
...
--13)Stored Procedure To Retrieve Orders by Customer ID
		CREATE PROCEDURE SP_GetOrderListByCustomer
						 @customer_id INT
		AS
		BEGIN
...
--14)Stored Procedure To Retrieve Stores by State
		CREATE PROCEDURE SP_GetStoreListByState
						 @state VARCHAR(25)
		AS
		BEGIN
...
----15)Stored Procedure To Retrieve Products by Brand with Count
		CREATE PROCEDURE SP_FindProductByBrand
						 @brand_name VARCHAR(255),
						 @product_count INT OUTPUT
		AS
		BEGIN
...
--16)Stored Procedure To Retrieve Orders by Status with Count
		CREATE PROCEDURE SP_FindOrdersByStatus
						 @order_status TINYINT,
...
--17)Stored Procedure To Retrieve Customers by City with Count
		CREATE PROCEDURE SP_FindCustomersByCity
			@city VARCHAR(255),
